...
{
    // Odom is GPS_init <--- GPS_curr
    // so, localPoseMap is GPS_init <--- GPS_curr
    // WGPS_T_WVIO is actually ENU <--- GPS_init, ENU is {x is East, y is North, z is Up}
    // so, globalPoseMap is ENU <--- GPS_curr
    // optimization outputs : 
    // (1) : WGPS_T_WVIO is updated
    // (2) : all elements in globalPoseMap is updated
