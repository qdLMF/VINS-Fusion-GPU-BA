...
void Estimator::optimization_with_ceres()
{
    ...
    if (last_marginalization_info && last_marginalization_info->valid)
    {
        ...
        // cuda_bundle_adjustment
        simple_marg_factor.Construct(last_marginalization_info);
        simple_marg_factor.valid = true;
        // cuda_bundle_adjustment
    }
    if(USE_IMU)
    {
    ...
    // cuda_bundle_adjustment
    if(simple_marg_factor.valid) {
        assert(last_marginalization_parameter_blocks.size() == simple_marg_factor.keep_block_data.size());

        simple_marg_factor.keep_block_name.clear();
        for(double* ptr : last_marginalization_parameter_blocks) {
            if( ptr == &(para_Ex_Pose[0][0]) )       { simple_marg_factor.keep_block_name.push_back("para_Ex_Pose[0]");    continue; }
            if( ptr == &(para_Ex_Pose[1][0]) )       { simple_marg_factor.keep_block_name.push_back("para_Ex_Pose[1]");    continue; }

            if( ptr == &(para_Td[0][0]) )            { simple_marg_factor.keep_block_name.push_back("para_Td[0]");         continue; }

            if( ptr == &(para_Pose[ 0][0]) )         { simple_marg_factor.keep_block_name.push_back("para_Pose[0]");       continue; }
            if( ptr == &(para_Pose[ 1][0]) )         { simple_marg_factor.keep_block_name.push_back("para_Pose[1]");       continue; }
            if( ptr == &(para_Pose[ 2][0]) )         { simple_marg_factor.keep_block_name.push_back("para_Pose[2]");       continue; }
            if( ptr == &(para_Pose[ 3][0]) )         { simple_marg_factor.keep_block_name.push_back("para_Pose[3]");       continue; }
            if( ptr == &(para_Pose[ 4][0]) )         { simple_marg_factor.keep_block_name.push_back("para_Pose[4]");       continue; }
            if( ptr == &(para_Pose[ 5][0]) )         { simple_marg_factor.keep_block_name.push_back("para_Pose[5]");       continue; }
            if( ptr == &(para_Pose[ 6][0]) )         { simple_marg_factor.keep_block_name.push_back("para_Pose[6]");       continue; }
            if( ptr == &(para_Pose[ 7][0]) )         { simple_marg_factor.keep_block_name.push_back("para_Pose[7]");       continue; }
            if( ptr == &(para_Pose[ 8][0]) )         { simple_marg_factor.keep_block_name.push_back("para_Pose[8]");       continue; }
            if( ptr == &(para_Pose[ 9][0]) )         { simple_marg_factor.keep_block_name.push_back("para_Pose[9]");       continue; }
            if( ptr == &(para_Pose[10][0]) )         { simple_marg_factor.keep_block_name.push_back("para_Pose[10]");      continue; }
            // if( ptr == &(para_Pose[11][0]) )         { simple_marg_factor.keep_block_name.push_back("para_Pose[11]");      continue; }
            // if( ptr == &(para_Pose[12][0]) )         { simple_marg_factor.keep_block_name.push_back("para_Pose[12]");      continue; }
            // if( ptr == &(para_Pose[13][0]) )         { simple_marg_factor.keep_block_name.push_back("para_Pose[13]");      continue; }
            // if( ptr == &(para_Pose[14][0]) )         { simple_marg_factor.keep_block_name.push_back("para_Pose[14]");      continue; }
            // if( ptr == &(para_Pose[15][0]) )         { simple_marg_factor.keep_block_name.push_back("para_Pose[15]");      continue; }
            // if( ptr == &(para_Pose[16][0]) )         { simple_marg_factor.keep_block_name.push_back("para_Pose[16]");      continue; }
            // if( ptr == &(para_Pose[17][0]) )         { simple_marg_factor.keep_block_name.push_back("para_Pose[17]");      continue; }
            // if( ptr == &(para_Pose[18][0]) )         { simple_marg_factor.keep_block_name.push_back("para_Pose[18]");      continue; }
            // if( ptr == &(para_Pose[19][0]) )         { simple_marg_factor.keep_block_name.push_back("para_Pose[19]");      continue; }
            // if( ptr == &(para_Pose[20][0]) )         { simple_marg_factor.keep_block_name.push_back("para_Pose[20]");      continue; }

            if( ptr == &(para_SpeedBias[ 0][0]) )    { simple_marg_factor.keep_block_name.push_back("para_SpeedBias[0]");  continue; }
            if( ptr == &(para_SpeedBias[ 1][0]) )    { simple_marg_factor.keep_block_name.push_back("para_SpeedBias[1]");  continue; }
            if( ptr == &(para_SpeedBias[ 2][0]) )    { simple_marg_factor.keep_block_name.push_back("para_SpeedBias[2]");  continue; }
            if( ptr == &(para_SpeedBias[ 3][0]) )    { simple_marg_factor.keep_block_name.push_back("para_SpeedBias[3]");  continue; }
            if( ptr == &(para_SpeedBias[ 4][0]) )    { simple_marg_factor.keep_block_name.push_back("para_SpeedBias[4]");  continue; }
            if( ptr == &(para_SpeedBias[ 5][0]) )    { simple_marg_factor.keep_block_name.push_back("para_SpeedBias[5]");  continue; }
            if( ptr == &(para_SpeedBias[ 6][0]) )    { simple_marg_factor.keep_block_name.push_back("para_SpeedBias[6]");  continue; }
            if( ptr == &(para_SpeedBias[ 7][0]) )    { simple_marg_factor.keep_block_name.push_back("para_SpeedBias[7]");  continue; }
            if( ptr == &(para_SpeedBias[ 8][0]) )    { simple_marg_factor.keep_block_name.push_back("para_SpeedBias[8]");  continue; }
            if( ptr == &(para_SpeedBias[ 9][0]) )    { simple_marg_factor.keep_block_name.push_back("para_SpeedBias[9]");  continue; }
            if( ptr == &(para_SpeedBias[10][0]) )    { simple_marg_factor.keep_block_name.push_back("para_SpeedBias[10]"); continue; }
            // if( ptr == &(para_SpeedBias[11][0]) )    { simple_marg_factor.keep_block_name.push_back("para_SpeedBias[11]"); continue; }
            // if( ptr == &(para_SpeedBias[12][0]) )    { simple_marg_factor.keep_block_name.push_back("para_SpeedBias[12]"); continue; }
            // if( ptr == &(para_SpeedBias[13][0]) )    { simple_marg_factor.keep_block_name.push_back("para_SpeedBias[13]"); continue; }
            // if( ptr == &(para_SpeedBias[14][0]) )    { simple_marg_factor.keep_block_name.push_back("para_SpeedBias[14]"); continue; }
            // if( ptr == &(para_SpeedBias[15][0]) )    { simple_marg_factor.keep_block_name.push_back("para_SpeedBias[15]"); continue; }
            // if( ptr == &(para_SpeedBias[16][0]) )    { simple_marg_factor.keep_block_name.push_back("para_SpeedBias[16]"); continue; }
            // if( ptr == &(para_SpeedBias[17][0]) )    { simple_marg_factor.keep_block_name.push_back("para_SpeedBias[17]"); continue; }
            // if( ptr == &(para_SpeedBias[18][0]) )    { simple_marg_factor.keep_block_name.push_back("para_SpeedBias[18]"); continue; }
            // if( ptr == &(para_SpeedBias[19][0]) )    { simple_marg_factor.keep_block_name.push_back("para_SpeedBias[19]"); continue; }
            // if( ptr == &(para_SpeedBias[20][0]) )    { simple_marg_factor.keep_block_name.push_back("para_SpeedBias[20]"); continue; }

            for(int i = 0; i < NUM_OF_F; i++) {
                if( ptr == &(para_Feature[i][0]) ) {
                    simple_marg_factor.keep_block_name.push_back("para_Feature[i]"); continue;
                }
            }
        }

        simple_marg_factor.PadJacAndRes(WINDOW_SIZE + 1);
    }
    // cuda_bundle_adjustment
    ...
    // if(simple_marg_factor.valid) {
    //     CUDABADataType weight_prior = 1.0;
    //     cuda_bundle_adjustment.host_Jprior = simple_marg_factor.jac_padded.cast<CUDABADataType>();
    //     cuda_bundle_adjustment.host_Eprior = simple_marg_factor.res_padded.cast<CUDABADataType>();
    //     cuda_bundle_adjustment.host_Hprior = (weight_prior * weight_prior) * (cuda_bundle_adjustment.host_Jprior.transpose() * cuda_bundle_adjustment.host_Jprior);
    //     cuda_bundle_adjustment.host_Bprior = -1.0 * (weight_prior * weight_prior) * (cuda_bundle_adjustment.host_Jprior.transpose() * cuda_bundle_adjustment.host_Eprior);
    // } else {
    //     cuda_bundle_adjustment.SetPriorZero();
    // }
    if(!simple_marg_factor.valid) {
    ...
    std::cout << "Iterations : " << num_iterations << std::endl;
    std::cout << "duration ms : solve : { " << solve_duration << " }" << std::endl;
    std::cout << "duration ms : solve in 10 iterations : { " << (solve_duration / num_iterations) * 10.0 << " }" << std::endl;

    double2vector();
    //printf("frame_count: %d \n", frame_count);

    printf(
        "states : %f %f %f %f %f %f %f \n", 
        para_Pose[WINDOW_SIZE][0], para_Pose[WINDOW_SIZE][1], para_Pose[WINDOW_SIZE][2], 
        para_Pose[WINDOW_SIZE][3], para_Pose[WINDOW_SIZE][4], para_Pose[WINDOW_SIZE][5], para_Pose[WINDOW_SIZE][6]
    );
